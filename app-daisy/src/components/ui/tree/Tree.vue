...
<template>
  <div class="tree-container">
    <div class="tree bg-base-100 rounded shadow p-2">
    ...
    <!-- 右键菜单 -->
    <div v-if="contextMenuVisible" class="context-menu menu bg-base-200 rounded shadow w-44 p-1" :style="{
      position: 'fixed',
      left: contextMenuPosition.x + 'px',
      top: contextMenuPosition.y + 'px',
      zIndex: 1060
    }">
      <button v-if="contextMenuNode?.children && contextMenuNode.children.length > 0" class="menu-item btn btn-ghost w-full justify-start"
        @click="handleContextMenuAction('expand')">
        <i class="bi bi-chevron-down mr-2"></i>展开
      </button>
      <button v-if="contextMenuNode?.children && contextMenuNode.children.length > 0" class="menu-item btn btn-ghost w-full justify-start"
        @click="handleContextMenuAction('collapse')">
        <i class="bi bi-chevron-up mr-2"></i>收起
      </button>
      <div class="divider my-1"></div>
      <button class="menu-item btn btn-ghost w-full justify-start" @click="handleContextMenuAction('expandAll')">
        <i class="bi bi-chevron-down mr-2"></i>全部展开
      </button>
      <button class="menu-item btn btn-ghost w-full justify-start" @click="handleContextMenuAction('collapseAll')">
        <i class="bi bi-chevron-up mr-2"></i>全部收起
      </button>
    </div>
  </div>
</template>
